...
# Omborni bir vaqtning o'zida tasdiqlanayotgan buyurtmalardan himoya qiladi.
# TeleBot bir nechta handlerni parallel ishlatishi mumkin, shuning uchun
# qoldiqni tekshirish va ayirish bitta kritik operatsiya sifatida bajariladi.
ombor_lock = threading.Lock()
...
def buyurtma_uchun_omborni_tekshirish_va_ayirish(itemlar):
    """
    Bir nechta buyurtma bir vaqtda tasdiqlansa ham ombor qoldig'ini
    ortiqcha sotib yubormaslik uchun qoldiqni tekshiradi va ayiradi.

    Natija:
        (True, qoldiq_matni)   -> hammasi yetarli va ayirildi
        (False, xatolik_matni) -> hech narsa ayirilmadi
    """
    with ombor_lock:
        try:
            workbook = openpyxl.load_workbook(EXCEL_FILE)
            ...
            # Bir xil model buyurtma ichida bir necha marta kelib qolsa,
            # avval jami talabni hisoblaymiz.
            talablar = {}
            for item in itemlar:
                kalit = (item["kategoriya"], item["model"])
                talablar[kalit] = talablar.get(kalit, 0) + int(item["son"])

            # Excel qatorlarini topamiz.
            qatorlar = {}
            for row in sheet.iter_rows(min_row=1):
                kategoriya = row[1].value
                model = row[2].value
                if kategoriya is None or model is None:
                    continue
                kalit = (str(kategoriya).strip(), str(model).strip())
                if kalit in talablar:
                    qatorlar[kalit] = row

            # Avval hamma mahsulotning qoldig'ini tekshiramiz.
            yetishmayotgan = []
            yangi_qoldiqlar = {}
            for kalit, talab in talablar.items():
                row = qatorlar.get(kalit)
                if row is None:
                    yetishmayotgan.append(f"{kalit[1]} — Excel'da topilmadi")
                    continue

                try:
                    mavjud = int(row[3].value or 0)
                except (TypeError, ValueError):
                    mavjud = 0

                if talab > mavjud:
                    yetishmayotgan.append(
                        f"{kalit[1]} — kerak: {talab}, mavjud: {mavjud}"
                    )
                else:
                    yangi_qoldiqlar[kalit] = mavjud - talab

            # Bitta mahsulot yetmasa ham hech narsani ayirmaymiz.
            if yetishmayotgan:
                return False, "\n".join(yetishmayotgan)

            # Hammasi yetarli bo'lsa, endi bir martada yozamiz.
            for kalit, yangi_son in yangi_qoldiqlar.items():
                qatorlar[kalit][3].value = yangi_son

            workbook.save(EXCEL_FILE)

            qoldiq_matni = ""
            for kalit, yangi_son in yangi_qoldiqlar.items():
                qoldiq_matni += f"📦 {kalit[1]} — omborda qoldi: {yangi_son} dona\n"

            return True, qoldiq_matni

        except Exception as e:
            return False, f"Omborni yangilashda xatolik: {e}"
...
def admin_zakazni_tasdiqlash(call):
    ...
    buyurtma = kutilayotgan_buyurtmalar.get(buyurtma_id)
    if not buyurtma:
        bot.send_message(call.message.chat.id, "Bu buyurtma topilmadi — avval tasdiqlangan yoki bekor qilingan bo'lishi mumkin.")
        return

    # Qoldiqni tekshirish + ayirish bitta himoyalangan operatsiya.
    muvaffaqiyatli, ombor_natijasi = buyurtma_uchun_omborni_tekshirish_va_ayirish(
        buyurtma["itemlar"]
    )
    ...
    if not muvaffaqiyatli:
        bot.send_message(
            call.message.chat.id,
            f"⚠️ Buyurtma #{buyurtma_id} hozircha tasdiqlanmadi. "
            f"Omborda yetarli mahsulot yo'q yoki xatolik yuz berdi.\n\n{ombor_natijasi}"
        )
        bot.send_message(
            buyurtma["chat_id"],
            f"⚠️ Buyurtmangiz #{buyurtma_id} hozircha tasdiqlanmadi. "
            f"Ombordagi qoldiq yetarli emas. Operator siz bilan bog'lanadi."
        )
        return

    # Ombor muvaffaqiyatli ayirilgandan keyingina kutish ro'yxatidan chiqaramiz.
    kutilayotgan_buyurtmalar.pop(buyurtma_id, None)
    ombor_matni = ombor_natijasi
